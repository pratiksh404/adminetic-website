<div>
    @isset($features)
    <ul wire:sortable="updateFeatureOrder">
        @foreach ($features as $feature)
        <li wire:sortable.item="{{ $feature->id }}" wire:key="feature-{{ $feature->id }}">
            <span class="text-muted" wire:sortable.handle style="cursor:move">{{ $feature->name }}</span>
        </li>
        @endforeach
    </ul>
    @endisset
