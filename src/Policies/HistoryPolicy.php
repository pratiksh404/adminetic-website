<?php

namespace Adminetic\Website\Policies;

use Adminetic\Website\Models\Admin\History;
use App\Models\User;
use Illuminate\Auth\Access\HandlesAuthorization;

class HistoryPolicy
{
    use HandlesAuthorization;

    public function before($user, $ability)
    {
        if ($user->isSuperAdmin()) {
            return true;
        }
    }

    /**
     * Determine whether the user can view any models.
     *
     * @param  \App\Models\User  $user
     * @return mixed
     */
    public function viewAny(User $user)
    {
        return $user->userCanDo('History', 'browse');
    }

    /**
     * Determine whether the user can view the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Admin\History  $history
     * @return mixed
     */
    public function view(User $user, History $history)
    {
        return $user->userCanDo('History', 'read');
    }

    /**
     * Determine whether the user can create models.
     *
     * @param  \App\Models\User  $user
     * @return mixed
     */
    public function create(User $user)
    {
        return $user->userCanDo('History', 'add');
    }

    /**
     * Determine whether the user can update the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Admin\History  $history
     * @return mixed
     */
    public function update(User $user, History $history)
    {
        return $user->userCanDo('History', 'edit');
    }

    /**
     * Determine whether the user can delete the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Admin\History  $history
     * @return mixed
     */
    public function delete(User $user, History $history)
    {
        return $user->userCanDo('History', 'delete');
    }

    /**
     * Determine whether the user can restore the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Admin\History  $history
     * @return mixed
     */
    public function restore(User $user, History $history)
    {
        return true;
    }

    /**
     * Determine whether the user can permanently delete the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Admin\History  $history
     * @return mixed
     */
    public function forceDelete(User $user, History $history)
    {
        return true;
    }
}
