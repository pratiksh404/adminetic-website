...
namespace Adminetic\Website\Http\Livewire\Admin\Process;
...
class ProcessTable extends DataTableComponent
{
    ...
    {
        Process::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No process found');
    ...
    public function reorder($items): void
    {
        foreach ($items as $item) {
            Process::find((int)$item['value'])->update(['position' => (int)$item['order']]);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Action")
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="process" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
