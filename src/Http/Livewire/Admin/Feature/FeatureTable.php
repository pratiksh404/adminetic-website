...
class FeatureTable extends DataTableComponent
{
    ...
    {
        Feature::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No feature found');
    ...
    public function reorder($items): void
    {
        foreach ($items as $item) {
            Feature::find((int) $item['value'])->update(['position' => (int) $item['order']]);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Action')
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="feature" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
