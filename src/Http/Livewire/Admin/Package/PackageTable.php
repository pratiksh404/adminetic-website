...
class PackageTable extends DataTableComponent
{
    ...
    {
        Package::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No package found');
    ...
    public function reorder($items): void
    {
        foreach ($items as $item) {
            Package::find((int)$item['value'])->update(['position' => (int)$item['order']]);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Action")
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="package" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
